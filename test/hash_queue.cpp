...
BOOST_AUTO_TEST_CASE(hash_queue__empty__no_checkpoints__true)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE(hashes.empty());
}

BOOST_AUTO_TEST_CASE(hash_queue__empty__one_checkpoint__true)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE(hashes.empty());
}

BOOST_AUTO_TEST_CASE(hash_queue__empty_size__initialize__false)
{
    hash_queue hashes(no_checks);
...
BOOST_AUTO_TEST_CASE(hash_queue__empty__initialize_enqueue_1__false)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
...
BOOST_AUTO_TEST_CASE(hash_queue__empty__initialize_dequeue__true)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
...
BOOST_AUTO_TEST_CASE(hash_queue__size__no_checkpoints__0)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE_EQUAL(hashes.size(), 0u);
}

BOOST_AUTO_TEST_CASE(hash_queue__size__one_checkpoint__0)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE_EQUAL(hashes.size(), 0u);
}

BOOST_AUTO_TEST_CASE(hash_queue__size__initialize__1)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
...
BOOST_AUTO_TEST_CASE(hash_queue__size__initialize_enqueue_1__2)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.enqueue(message_factory(1, check42.hash())));
...
// The chain is broken when the list is emptied.
BOOST_AUTO_TEST_CASE(hash_queue__size__initialize_dequeue__0)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.dequeue());
...
BOOST_AUTO_TEST_CASE(hash_queue__first_height__no_checkpoints__default)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE_EQUAL(hashes.first_height(), 0u);
}

BOOST_AUTO_TEST_CASE(hash_queue__first_height__one_checkpoint__default)
{
    hash_queue hashes(one_check);
    BOOST_REQUIRE_EQUAL(hashes.first_height(), 0u);
}

BOOST_AUTO_TEST_CASE(hash_queue__first_height__initialize__initial)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
...
BOOST_AUTO_TEST_CASE(hash_queue__first_height__initialize_enqueue_1__initial)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.enqueue(message_factory(1, check42.hash())));
...
// The chain is broken when the list is emptied.
BOOST_AUTO_TEST_CASE(hash_queue__first_height__initialize_dequeue__initial_plus_1)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.dequeue());
...
BOOST_AUTO_TEST_CASE(hash_queue__flast_height__no_checkpoints__default)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE_EQUAL(hashes.last_height(), 0u);
}

BOOST_AUTO_TEST_CASE(hash_queue__last_height__one_checkpoint__default)
{
    hash_queue hashes(one_check);
    BOOST_REQUIRE_EQUAL(hashes.last_height(), 0u);
}

BOOST_AUTO_TEST_CASE(hash_queue__last_height__initialize__initial)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
...
BOOST_AUTO_TEST_CASE(hash_queue__last_height__initialize_enqueue_1__initial_plus_1)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.enqueue(message_factory(1, check42.hash())));
...
// The chain is broken when the list is emptied.
BOOST_AUTO_TEST_CASE(hash_queue__last_height__initialize_dequeue__initial_plus_1)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.dequeue());
...
BOOST_AUTO_TEST_CASE(hash_queue__last_hash__no_checkpoints__null_hash)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE(hashes.last_hash() == null_hash);
}

BOOST_AUTO_TEST_CASE(hash_queue__last_hash__one_checkpoint__null_hash)
{
    hash_queue hashes(one_check);
    BOOST_REQUIRE(hashes.last_hash() == null_hash);
}

BOOST_AUTO_TEST_CASE(hash_queue__last_hash__initialize__expected)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
...
BOOST_AUTO_TEST_CASE(hash_queue__last_hash__initialize_enqueue_1__expected)
{
...
// The chain is broken when the list is emptied.
BOOST_AUTO_TEST_CASE(hash_queue__last_hash__initialize_dequeue__null_hash)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.dequeue());
...
BOOST_AUTO_TEST_CASE(hash_queue__dequeue1__empty_dequeue_0__true)
{
    hash_queue hashes(no_checks);
    BOOST_REQUIRE(hashes.dequeue(0));
}

BOOST_AUTO_TEST_CASE(hash_queue__dequeue1__empty_dequeue_1__false)
{
    hash_queue hashes(no_checks);
...
// The chain is broken when the list is emptied.
BOOST_AUTO_TEST_CASE(hash_queue__dequeue1__enqueue_1_dequeue_1__true_empty)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.dequeue(1));
...
// The chain is broken when the list is emptied.
BOOST_AUTO_TEST_CASE(hash_queue__dequeue1__size_exceeded__false_empty)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__dequeue2__empty__false)
{
    hash_queue hashes(no_checks);
    size_t height;
    hash_digest hash;
...
BOOST_AUTO_TEST_CASE(hash_queue__dequeue2__initialize__true_expected)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    size_t height;
...
BOOST_AUTO_TEST_CASE(hash_queue__dequeue2__initialize_enqueue_1__true_expected)
{
    const auto message = message_factory(1, check42.hash());
...
BOOST_AUTO_TEST_CASE(hash_queue__dequeue2__initialize_enqueue_1_dequeue__true_expected)
{
    const auto message = message_factory(1, check42.hash());
    const auto expected = message->elements[0].hash();
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__enqueue_1__false_empty)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__initialize_enqueue_0__true_size_1)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__initialize_enqueue_1__size_2)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE(hashes.enqueue(message_factory(1, check42.hash())));
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__initialize_enqueue_1_dequeue_enqueue_2_dequeue__expected)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__linked__true_expected_order)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__unlinked__false_expected)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__checkpoint_match__true_expected)
{
    const auto message = message_factory(2, check42.hash());
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__single_checkpoint_mismatch__false_rollback_to_initial)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__enqueue__multiple_checkpoint_mismatch__false_rollback_to_preceding_checkpoint)
{
...
BOOST_AUTO_TEST_CASE(hash_queue__initialize1__always__size_1)
{
    hash_queue hashes(no_checks);
    hashes.initialize(check42);
    BOOST_REQUIRE_EQUAL(hashes.size(), 1u);
...
BOOST_AUTO_TEST_CASE(hash_queue__initialize2__always__size_1)
{
    hash_queue hashes(no_checks);
