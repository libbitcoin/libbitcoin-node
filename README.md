[![Build Status](https://github.com/libbitcoin/libbitcoin-node/actions/workflows/ci.yml/badge.svg?branch=version3)](https://github.com/libbitcoin/libbitcoin-node/actions/workflows/ci.yml)

[![Coverage Status](https://coveralls.io/repos/libbitcoin/libbitcoin-node/badge.svg?branch=version3)](https://coveralls.io/r/libbitcoin/libbitcoin-node?branch=version3)

# Libbitcoin Node

*The Bitcoin full node based on libbitcoin-blockchain*
...
On Linux and macOS libbitcoin-node can be build using Autotools and cmake. This process is accomplished via the provided installation scripts `install.sh` or `install-cmake.sh`. These scripts provide a basis for installation of the dependencies of the project. Both support the use of the argument `--help` for further parameterization.

Installation of libbitcoin-node comes with requirements listed below.
...
A minimal libbitcoin-node build requires boost 1.72.0 to 1.76.0 and libsecp256k1. The [libbitcoin/secp256k1](https://github.com/libbitcoin/secp256k1/tree/version7) repository is forked from [bitcoin-core/secp256k1](https://github.com/bitcoin-core/secp256k1) in order to control for changes and to incorporate the necessary Visual Studio build. The original repository can be used directly but recent changes to the public interface may cause build breaks. The `--enable-module-recovery` switch is required. Both of these are able to be provided via the installation script(s).
...
Below is a breakdown of preparation required to install libbitcoin-node. Once an appropriate [compiler](http://en.wikipedia.org/wiki/Compiler) and [build system](http://wikipedia.org/wiki/GNU_build_system) are in place, proceed to [Install Script Execution](#installscript) below to perform installation.
...
Next install the [Boost](http://www.boost.org) (1.72.0 - 1.76.0) development package. This can be accomplished via parameterization of the installation script during libbitcoin-node installation and it is recommended to the reader that this approach be used.
...
The [autotools install script](https://github.com/libbitcoin/libbitcoin-node/blob/version3/install.sh) and [cmake install script](https://github.com/libbitcoin/libbitcoin-node/blob/version3/install-cmake.sh) are written so that the manual build steps for each dependency can be inferred by a developer.

You can run either install script from any directory on your system. By default this will build libbitcoin-node in a subdirectory named `build-libbitcoin-node` and install it to `/usr/local/`. The install script requires `sudo` only if you do not have access to the installation location, which you can change using the `--prefix` option on the installer command line.

The build script clones, builds and installs seven unpackaged repositories, namely:
...
- [libbitcoin/libbitcoin-node](https://github.com/libbitcoin/libbitcoin-node)
