...
chaser_check::~chaser_check() NOEXCEPT
{
    ...
    if (!stopped()) { LOGF("~chaser_check is not stopped."); }
}

void chaser_check::start(network::result_handler&& handler) NOEXCEPT
{
    if (!stopped())
    {
...
void chaser_check::stop() NOEXCEPT
{
    stopped_.store(true);

    // The chaser_check can be deleted once threadpool joins after this call.
    boost::asio::post(strand_,
        std::bind(&chaser_check::do_stop, this));
}

chaser_check::object_key chaser_check::subscribe(notifier&& handler) NOEXCEPT
{
...
bool chaser_check::notify(object_key key) NOEXCEPT
{
    return subscriber_.notify_one(key, network::error::success);
}

bool chaser_check::stopped() const NOEXCEPT
{
    return stopped_.load();
}

bool chaser_check::stranded() const NOEXCEPT
{
    return strand_.running_in_this_thread();
}

// private
chaser_check::object_key chaser_check::create_key() NOEXCEPT
{
    BC_ASSERT_MSG(stranded(), "strand");
...
// private
void chaser_check::do_stop() NOEXCEPT
{
    BC_ASSERT_MSG(stranded(), "strand");
