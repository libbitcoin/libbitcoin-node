...
chaser_connect::~chaser_connect() NOEXCEPT
{
    ...
    if (!stopped()) { LOGF("~chaser_connect is not stopped."); }
}

void chaser_connect::start(network::result_handler&& handler) NOEXCEPT
{
    if (!stopped())
    {
...
void chaser_connect::stop() NOEXCEPT
{
    stopped_.store(true);

    // The chaser_connect can be deleted once threadpool joins after this call.
    boost::asio::post(strand_,
        std::bind(&chaser_connect::do_stop, this));
}

chaser_connect::object_key chaser_connect::subscribe(notifier&& handler) NOEXCEPT
{
...
bool chaser_connect::notify(object_key key) NOEXCEPT
{
    return subscriber_.notify_one(key, network::error::success);
}

bool chaser_connect::stopped() const NOEXCEPT
{
    return stopped_.load();
}

bool chaser_connect::stranded() const NOEXCEPT
{
    return strand_.running_in_this_thread();
}

// private
chaser_connect::object_key chaser_connect::create_key() NOEXCEPT
{
    BC_ASSERT_MSG(stranded(), "strand");
...
// private
void chaser_connect::do_stop() NOEXCEPT
{
    BC_ASSERT_MSG(stranded(), "strand");
